#!/usr/bin/env bash
# d12 XSA runner. Runs INSIDE a RunPod pod.
# Same pipeline as d12.sh (tokenizer -> base_train -> base_eval -> SFT -> chat_eval -> report)
# but with XSA enabled and a separate HF repo / wandb run name so artifacts don't
# commingle with the d12 baseline.
# On exit:
...
# Optional env:
#   WANDB_RUN              default: d12-xsa
#   NANOCHAT_REPO          default: Team-XSA/nanochat
#   NANOCHAT_REF           default: dev
#   HF_REPO                default: haydenfree/nanochat-d12-xsa
...
WANDB_RUN="${WANDB_RUN:-d12-xsa}"
BACKUP_INTERVAL="${BACKUP_INTERVAL:-300}"
UPLOAD_FAILURE_CACHE="${UPLOAD_FAILURE_CACHE:-0}"
# XSA / VE defaults — overridable from kickoff env. d12_xsa defaults XSA on.
XSA="${XSA:-TRUE}"
...
# the previous d12_xsa run). The background backup loop is best-effort; this is not.
